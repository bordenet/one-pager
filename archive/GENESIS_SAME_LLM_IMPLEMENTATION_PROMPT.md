# Genesis Same-LLM Implementation Prompt
...
CRITICAL REQUIREMENT: When Phase 1 and Phase 2 use the same LLM (detected via matching URLs, endpoints, or provider/model), automatically augment Phase 2 prompts with Gemini personality simulation to maintain adversarial tension.
...
VALIDATION: Mirror the test suite from tests/same-llm-adversarial.test.js to ensure all 13 scenarios work correctly.
...
## 📋 **Specific Files to Inspect and Mirror**

1. **Core Implementation**: `same_llm_adversarial_implementation.js`
2. **Test Suite**: `tests/same-llm-adversarial.test.js` (13 test scenarios)
3. **Implementation Guide**: `SAME_LLM_IMPLEMENTATION_SUMMARY.md`
...
This prompt directs Claude to inspect our actual working implementation and incorporate it into Genesis bootstrapping for automatic same-LLM support in all new projects.
