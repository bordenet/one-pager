/**
 * One-Pager Prompts Module
 * @module prompts
 *
 * Manages workflow configuration and prompt generation for one-pager documents.
...
export async function generatePhase1Prompt(formData) {
  ...
  return replaceTemplateVars(template, {
    ...
    COST_OF_DOING_NOTHING: formData.costOfDoingNothing || '',
    CONTEXT: formData.context || '',
    PROPOSED_SOLUTION: formData.proposedSolution || '',
    KEY_GOALS: formData.keyGoals || '',
    SCOPE_IN_SCOPE: formData.scopeInScope || '',
    SCOPE_OUT_OF_SCOPE: formData.scopeOutOfScope || '',
    SUCCESS_METRICS: formData.successMetrics || '',
    KEY_STAKEHOLDERS: formData.keyStakeholders || '',
    TIMELINE_ESTIMATE: formData.timelineEstimate || ''
  });
}

/**
 * Phase 2 Prompt: Gemini Review
