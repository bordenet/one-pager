...
test.describe("One-Pager Assistant", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.evaluate(() => {
      localStorage.clear();
      indexedDB.deleteDatabase("one-pager-assistant");
    });
  ...
  test("should load the application", async ({ page }) => {
    ...
    expect(title).toContain("One-Pager Assistant");
  });
  ...
  test("should create a new project", async ({ page }) => {
    ...
    if (await createBtn.isVisible()) {
      ...
      const problemInput = await page.locator("[id*='problem']").first();
  ...
  test("should display correct header title", async ({ page }) => {
    ...
    expect(text).toContain("One-Pager Assistant");
  });
